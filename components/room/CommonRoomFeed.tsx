...
export function CommonRoomFeed({ initialPosts, readMap = {}, commentCountsMap = {} }: CommonRoomFeedProps) {
    ...
    <div className="max-w-[1200px] mx-auto px-6 py-8">
      ...
      <div className="mb-8">
        ...
        <p className="mt-2 font-sans text-sm text-text-secondary">
          Essays, stories, poems, reflections, and other written works shared by the Fellowship.
        </p>
        {initialPosts.length > 0 && (
          <p className="mt-1.5 font-sans text-xs text-text-muted">
            {initialPosts.length} {initialPosts.length === 1 ? 'post' : 'posts'} · {new Set(initialPosts.map(p => p.user_id)).size} {new Set(initialPosts.map(p => p.user_id)).size === 1 ? 'writer' : 'writers'}
          </p>
        )}
      </div>
